...
const Openings = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (enabledOpenings.length > 0) {
          // If access list is available for the student, place allowed openings first
          if (!isAdmin && contentAccess?.openingAccess?.allowedOpenings?.length) {
            ...
            enabledOpenings.sort((a, b) => {
              ...
              return aAllowed - bAllowed; // allowed (0) come first
            });
          }
  ...
  // Also reorder already-loaded openings when contentAccess becomes available
  useEffect(() => {
    if (!isAdmin && contentAccess?.openingAccess?.allowedOpenings?.length && openings.length > 0) {
      const allowed = new Set(contentAccess.openingAccess.allowedOpenings.map(String));

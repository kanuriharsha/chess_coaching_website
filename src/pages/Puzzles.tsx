...
const Puzzles = () => {
  ...
  const [moveHistory, setMoveHistory] = useState<HistoryStep[]>([]); // Full move history for Back/Next navigation
  const [historyIndex, setHistoryIndex] = useState<number>(0); // Current index in moveHistory being viewed
    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        if (move) {
          ...
          const initialStep: HistoryStep = { fen: currentPuzzle.fen, label: 'Start position' };
          const preloadedStep: HistoryStep = {
            fen: gameCopy.fen(),
            san: move.san,
            from,
            to,
            label: `Opponent: ${move.san}`
          };
          setMoveHistory([initialStep, preloadedStep]);
          setHistoryIndex(1);
  ...
  const handleMove = useCallback(
    (from: Square, to: Square): boolean => {
      ...
      const playerStep: HistoryStep = {
        fen: gameCopy.fen(),
        san: move.san,
        from: fromSq,
        to: toSq,
        label: `Your move: ${move.san}`
      };
        ...
        if (opponentCandidates.length === 0) {
          ...
          setTimeout(() => {
            ...
            try {
              ...
              if (oppMove) {
                const oppFrom = oppMove.from as Square;
                const oppTo = oppMove.to as Square;
                setGame(updatedGame);
                setLastMove({ from: oppFrom, to: oppTo });
                setCurrentNodeId(opponentNode.id);
                playSound('move');
                const oppStep: HistoryStep = {
                  fen: updatedGame.fen(),
                  san: oppMove.san,
                  from: oppFrom,
                  to: oppTo,
                  label: `Opponent: ${oppMove.san}`
                };
                setMoveHistory(prev => {
                  const nextHist = [...prev, playerStep, oppStep];
                  setHistoryIndex(nextHist.length - 1);
                  return nextHist;
                });
        ...
        setTimeout(() => {
          ...
          if (nextMove) {
            ...
            try {
              ...
              if (opponentMove) {
                const oppFrom = opponentMove.from as Square;
                const oppTo = opponentMove.to as Square;
                setGame(updatedGame);
                setLastMove({ from: oppFrom, to: oppTo });
                ...
                const oppStep: HistoryStep = {
                  ...
                  from: oppFrom,
                  to: oppTo,
                  label: `Opponent: ${opponentMove.san}`
                };
                setMoveHistory(prev => {
                  const nextHist = [...prev, playerStep, oppStep];
                  setHistoryIndex(nextHist.length - 1);
  ...
  // Move Navigation Handlers for students (only navigates moves already made)
  const handleBackMove = useCallback(() => {
    if (historyIndex <= 0 || moveHistory.length === 0) return;
    const prevIdx = historyIndex - 1;
    setHistoryIndex(prevIdx);
    const step = moveHistory[prevIdx];
    setGame(new Chess(step.fen));
    setLastMove(step.from && step.to ? { from: step.from, to: step.to } : null);
    playSound('move');
  }, [historyIndex, moveHistory, playSound]);

  const handleNextMove = useCallback(() => {
    // Only advance through moves already recorded in history (never reveals solution)
    if (historyIndex < moveHistory.length - 1) {
      const nextIdx = historyIndex + 1;
      setHistoryIndex(nextIdx);
      const step = moveHistory[nextIdx];
      setGame(new Chess(step.fen));
      setLastMove(step.from && step.to ? { from: step.from, to: step.to } : null);
      playSound('move');
    }
  ...
  // Keyboard listener for Left and Right arrow keys (for laptop/keyboard users)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Don't trigger if reorder mode is active or promotion dialog is open
      if (isReorderMode || showPromotion) {
        return;
      }

      // Only handle when viewing a puzzle
  ...
  }, [currentPuzzle, isReorderMode, showPromotion, historyIndex, moveHistory, handleBackMove, handleNextMove]);
              ...
              <div className="relative flex items-center justify-center">
                ...
                <div className="w-full max-w-2xl aspect-square">
                  <ChessBoard
                    ...
                    interactive={!solved && !currentPuzzle?.isLocked && historyIndex === moveHistory.length - 1}
                    lastMove={lastMove}
                  />
                  <PromotionDialog
                    open={showPromotion}
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="card-premium p-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {/* Move Navigation: Back and Next Buttons */}
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={handleBackMove}
                        disabled={historyIndex <= 0}
                        className="flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-secondary text-secondary-foreground hover:bg-secondary/80 disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                        title="Back to previous move (Left Arrow key)"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-secondary text-secondary-foreground hover:bg-secondary/80 disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                        title="Next move (Right Arrow key)"
                      >
                        Next
                    ...
                    {/* Jump to latest button when reviewing past moves */}
                    {historyIndex < moveHistory.length - 1 && (
                      <button
                        onClick={() => {
                          const lastIdx = moveHistory.length - 1;
                          setHistoryIndex(lastIdx);
                          setGame(new Chess(moveHistory[lastIdx].fen));
                          setLastMove(moveHistory[lastIdx].from && moveHistory[lastIdx].to ? { from: moveHistory[lastIdx].from!, to: moveHistory[lastIdx].to! } : null);
                          playSound('move');
                        }}
                        className="w-full py-1.5 px-3 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 transition-colors text-xs font-medium flex items-center justify-center gap-1.5"
                      >
                        Jump to latest

...
const VisualBoardEditor: React.FC<VisualBoardEditorProps> = ({
  ...
  const recordMoveToTree = useCallback((san: string, gameAfterMove: Chess) => {
    const newId = genId();
    const newTree = [...moveTree, { id: newId, move: san, parentId: currentParentId }];
    setMoveTree(newTree);
    ...
    // Auto-sync with parent so state is never stale
    const mainLine = getMainLine(newTree);
    onPositionSave(setupFen, mainLine, preloadedMove || undefined, newTree);
  }, [currentParentId, moveTree, setupFen, preloadedMove, onPositionSave]);
  ...
  const deleteNodeSubtree = useCallback((nodeId: string) => {
    ...
    while (queue.length > 0) {
      ...
      toDelete.add(cur);
      moveTree.filter(n => n.parentId === cur).forEach(n => queue.push(n.id));
    }
    const newTree = moveTree.filter(n => !toDelete.has(n.id));
    setMoveTree(newTree);
    ...
    const baseFen = preloadedMoveFen || setupFen;
    const newFen = computeFenAtNode(newTree, newParentId, baseFen);
    setSolutionGame(new Chess(newFen));
  ...
  }, [moveTree, currentParentId, setupFen, preloadedMove, preloadedMoveFen, branchingFromNodeId, onPositionSave]);
  ...
  const undoLastMove = () => {
    ...
    // Delete lastNode AND all of its descendant nodes in the tree
    const toDelete = new Set<string>();
    const queue = [lastNode.id];
    while (queue.length > 0) {
      const cur = queue.shift()!;
      toDelete.add(cur);
      moveTree.filter(n => n.parentId === cur).forEach(n => queue.push(n.id));
    ...
    // Auto-sync clean state to parent immediately so backend never receives undone moves
    const mainLine = getMainLine(newTree);
    onPositionSave(setupFen, mainLine, preloadedMove || undefined, newTree);
    toast.info(`Move "${lastNode.move}" undone`);
  };
  ...
  const backToSetup = () => {
    ...
    setCurrentParentId(null);
    setBranchingFromNodeId(null);
    onPositionSave(setupFen, [], undefined, []);
  };
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (window.confirm('Clear all solution moves?')) {
                          ...
                          const baseFen = preloadedMoveFen || setupFen;
                          setSolutionGame(new Chess(baseFen));
                          onPositionSave(setupFen, [], preloadedMove || undefined, []);
                          toast.info('All solution moves cleared');
                        }
                      }}
